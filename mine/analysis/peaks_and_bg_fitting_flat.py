...
def initial_bg_poly(spec, shifts, order):
    ...
    residuals = [closest_arg(index, np.setdiff1d(bg_indices, index))[1] for index in bg_indices]
...
def run(spec,
    ...
    @njit()
    def line(H, C, W):
        return G(shifts, H, C, W)
    @jit()
    def _multi_line(parameters):
        return np.sum(np.array([line(*peak) for peak in parameters]), axis=0)

    print(_multi_line(np.array([[300,300,10],
                       [300, 300, 10]])))
    raise Exception
    def multi_line(parameters):
        if parameters:
            return _multi_line(np.array(parameters))
    ...
    def add_new_peak(shifts, signal, peaks, regions, noise_threshold, width, bound):
        '''
        lifted from Iterative_Raman_Fitting
        '''
        # -----Calc. size of x_axis regions-------
        sectionsize = (shifts.max()--shifts.min())/regions
        start = np.min(shifts)
        ...
        if not len(peaks):
            current = np.array(shifts)*0
        else:
            current = multi_line(peaks)
    
        def loss(params):
            # *multi_L(shifts,*peaks))# if this overlaps with another lorentzian it's biased against it
            return np.sum(np.abs(current+line(*params)-signal))
    
        for i in range(int(regions)):
            bounds = [(0, np.inf), (i*sectionsize+start, (i+1) *
                                    sectionsize+start), (0, max(shifts)-min(shifts))]
            Centre = (i+np.random.rand())*sectionsize+start
            try:
                Height = max(split(signal, shifts, i*sectionsize +
                                      start, (i+1)*sectionsize+start)[0])-min(signal)
            except:
                Height = noise_threshold
            params = [Height, Centre, width]
        ...
        for peak_candidate in sorted_results:
            ...
            if peak_candidate[0] > noise_threshold and maxwidth > peak_candidate[2] > minwidth:
                if len(peaks) != 0:
                    peak_index, residual = closest_arg(peak_candidate[1], np.transpose(peaks)[1])
                    # is far enough away from existing peaks
                    if residual > min_peak_spacing*peaks[peak_index][2]:
                        return peak_candidate.tolist()
    ...
    bg_p = initial_bg_poly(spec, shifts, order)  # takes a guess at the background
    def bg(p):
    ...
    def optimize_peaks(peaks, signal):
        ...
        if len(peaks) < 2:
            return
        peaks = peaks_to_matrix(minimize(peak_loss, np.ravel(peaks), bounds=bounds).x)
        return peaks
    ...
    def optimize_centre_and_width(peaks, signal):
        ...
        if len(peaks) < 2:
            return peaks
        heights = np.transpose(peaks)[0]
        centres_and_widths = np.transpose(peaks)[1:]
    ...
    while regions <= len(spec):
        if verbose == True:
            print('Region fraction: ', np.around(
                regions/float(len(spec)), decimals=2))
        existing_loss_score = loss_function(bg_p, peaks)  # measure of fit 'goodness'
       
        new_peak = add_new_peak(shifts, signal(bg_p), peaks, regions, noise_threshold, width, bound)  # adds a peak
        if new_peak:
        ...
        elif not len(peaks):  # if there wasn't a peak added, try harder
            regions *= 5
