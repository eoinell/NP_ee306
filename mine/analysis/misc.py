...
def split(wavelengths, spectrum, lower=-np.inf, upper=np.inf):
    ...
    split_spectrum = spectrum[condition] if len(spectrum.shape)<2 else np.array([s[condition] for s in spectrum])  
...
def condenseZscan(zScan, normalised = False):    
    ...
    if normalised: output /= float(max(output[(len(output),3):(2*len(output)/3)]))
    return output
