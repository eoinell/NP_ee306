...
def stokesratio(Stokes_counts, antiStokes_counts, shift, laser_wavelength = 785.):#returns T, shift is in cm-1
    ...
    logarg = (old_div(np.array(Stokes_counts),np.array(antiStokes_counts)))*(old_div(omega_AS,omega_S))**4
    T = old_div(scipy.constants.hbar*omega,(scipy.constants.k*np.log(logarg)))
...
def condenseZscan(zScan, normalised = False):    
    ...
    if normalised == True: output /= float(max(output[old_div(len(output),3):old_div(2*len(output),3)]))
    return output
...
def get_peak_heights(counts, wavelengths, shift, Range=4., return_wavelength = False, antiStokes = True, inputcm = True): # shift is in cm-1, range is in nm
    ...
    if inputcm==False:    
        ...
        S_group, S_group_wl = truncate(counts, wavelengths, S_nm-(old_div(Range,2)), S_nm+old_div((Range),2))
    else:
        S_group, S_group_wl = truncate(counts, wavelengths, shift-(old_div(Range,2)), shift+old_div((Range),2))
    ...
    if antiStokes ==True:
        if inputcm==False: 
            ...
            AS_group, AS_group_wl = truncate(counts, wavelengths, AS_nm-old_div(Range,2), AS_nm+old_div(Range,2))
        else:
            AS_group, AS_group_wl = truncate(counts, wavelengths, -shift-old_div(Range,2), -shift+old_div(Range,2))
...
def calibrate_Si(counts, wavelengths, shift=520, centre_wl = 785.):
    ...
    S_factor = old_div((cnv.cm_to_wavelength(-shift, centre_wl = centre_wl)-centre_wl),(S_wl-centre_wl))
    AS_factor = old_div((cnv.cm_to_wavelength(shift, centre_wl = centre_wl)-centre_wl),(AS_wl-centre_wl))    
